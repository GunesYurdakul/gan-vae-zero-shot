...
class generator(nn.Module):
    ...
    def __init__(self, input_dim=100, output_dim=1, input_size=32, class_num=8):
        ...
        self.fc = nn.Sequential(
            ...
            nn.BatchNorm1d(1024),
            nn.ReLU(),
            nn.Linear(1024, 128 * (self.input_size // 2)),
            nn.BatchNorm1d(128 *  (self.input_size // 2)),
            nn.ReLU(),
        )
...
class discriminator(nn.Module):
    ...
    def __init__(self, input_dim=1, output_dim=1, input_size=32, class_num=8):
        ...
        self.fc = nn.Sequential(
            nn.Linear(128 * (self.input_size // 2), 1024),
            nn.BatchNorm1d(1024),
            nn.LeakyReLU(0.2),
    ...
    def forward(self, input, label):
        print("shapes input and label:")
        print(input.shape)
        print(label.shape)
        print(self.input_size)
        x = torch.cat([input, label], 1)
        x = self.conv(x)
        x = x.view(-1, 128 * (self.input_size // 2))
        x = self.fc(x)
