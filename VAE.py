...
parser.add_argument('--one-shot', type=bool, default=False,
                    help='label to remove')
...
if args.dataset == 'mnist':
    ...
    if args.remove_label!=-1:
        ...
        if args.one_shot:
            ...
            if args.one_shot:
                train_loader.dataset.data =torch.cat((zero_shot_data, one_shot_data[:10]), 0)
                train_loader.dataset.targets=torch.cat((zero_shot_label, one_shot_label[:10]), 0)
...
elif args.dataset == 'bags':
    transform = transforms.Compose([transforms.Resize((args.input_size, args.input_size)), transforms.ToTensor(), transforms.Normalize((0.5,), (0.5,))])
    train_loader = torch.utils.data.DataLoader(datasets.ImageFolder('./data/bags/',transform=transform),batch_size=args.batch_size, shuffle=True)
    test_loader = torch.utils.data.DataLoader(datasets.ImageFolder('./data/bags/',transform=transform),batch_size=args.batch_size, shuffle=True)
        ...
        for i, (data, label) in enumerate(test_loader):
            ...
            if i == 0:
                ...
                save_image(comparison.cpu(),
                         'results/fashion-mnist/few-shot-reconstruction_' + str(epoch) + '.png', nrow=n)
                z_vector = z
                mu_vector = mu
                logvar_vector=logvar
                label_vector=label
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        with torch.no_grad():
            ...
            save_image(sample.view(64, 1, 28, 28),
                       'results/fashion-mnist/1_fewshot-sample_' + str(epoch) + '.png')
    print('will be saved')
    torch.save(model.state_dict(), 'model_trousers_fewshot_missing.pt')
    print("model saved")