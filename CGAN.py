...
class generator(nn.Module):
    ...
    def __init__(self, input_dim=100, output_dim=1, input_size=32, class_num=8):
        ...
        self.fc = nn.Sequential(
            ...
            nn.BatchNorm1d(1024),
            nn.ReLU(),
            nn.Linear(1024, 128 * self.input_size ),
            nn.BatchNorm1d(128 * self.input_size),
            nn.ReLU(),
        )
    ...
    def forward(self, input, label):
        ...
        x = self.fc(x)
        x = x.view(-1, 128, self.input_size)
        x = self.deconv(x)
...
class discriminator(nn.Module):
    ...
    def __init__(self, input_dim=1, output_dim=1, input_size=32, class_num=8):
        ...
        self.fc = nn.Sequential(
            nn.Linear(128 * self.input_size , 1024),
            nn.BatchNorm1d(1024),
            nn.LeakyReLU(0.2),
...
class CGAN(object):
    def __init__(self, args):
        ...
        if self.gpu_mode:
            self.sample_z_, self.sample_y_ = self.sample_z_.cpu(), self.sample_y_.cpu()
    ...
    def train(self):
        ...
        for epoch in range(self.epoch):
            ...
            for iter, (x_, y_) in enumerate(self.data_loader):
                ...
                y_fill_ = y_vec_.expand(self.batch_size, self.class_num)
                if self.gpu_mode:
                    x_, z_, y_vec_, y_fill_ = x_.cpu(), z_.cpu(), y_vec_.cpu(), y_fill_.cpu()
    ...
    def visualize_results(self, epoch, fix=True):
        ...
        if fix:
            ...
            if self.gpu_mode:
                sample_z_, sample_y_ = sample_z_.cpu(), sample_y_.cpu()
